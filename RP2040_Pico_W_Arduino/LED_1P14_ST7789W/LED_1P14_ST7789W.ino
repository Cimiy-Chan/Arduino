...
//The following definition is for RPI-Pico W with 1.14in LCD
//#define LCD_DC 8
//#define LCD_CS 9
//#define LCD_RST 12
...
void setup(void) {
  ...
  // large block of text
  tft.setRotation(3); //Value: 0-3 (0=0, 1=90, 2=180, 3=270)
  tft.setTextSize(2);
  tft.fillScreen(ST77XX_BLACK);
  testdrawtext("Testing....", ST77XX_WHITE);
  delay(1000);
...
void loop() {
  ...
  counter ++;
  if (counter > 3) {
    Serial.println("End... LCD off");
    tft.enableDisplay(false);
    while (true) { //Death loop with only LED flash, LCD off
      digitalWrite(BI_LED, LOW);
      delay (500);
      digitalWrite(BI_LED, HIGH);
      delay (500);      
